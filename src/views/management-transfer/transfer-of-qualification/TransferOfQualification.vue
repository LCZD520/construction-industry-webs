...
*/
<template>
  <div class="transfer-of-qualification">
    <el-form
        ref="form"
        :rules="rules"
        inline
...
export default {
  ...
  methods: {
    handleView(_id) {
      this.$router.push(`/transfer-qualification-transfer-view/${_id}`)
    },
    async search(pageSize) {
      ...
      this.loading = true
      try {
        const res = await this.$http.post('/qualification-transfer-transfer/list', newForm)
        if (res.status) {
    ...
    handleAudit(_id) {
      this.$router.push(`/qualification-transfer-transfer/audit/${_id}`)
    },
  }
}
</script>
