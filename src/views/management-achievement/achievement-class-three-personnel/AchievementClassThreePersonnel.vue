...
*/
<template>
  <div class="achievement-class-three-personnel">
    <el-form
        size="small"
        ref="form"
        inline
      ...
      <el-form-item label="订单编号" prop="orderno">
        <el-input v-model.trim="form.orderno" placeholder="请输入学历提升订单编号">
        </el-input>
      </el-form-item>
...
<script>
export default {
  name: 'AchievementClassThreePersonnel',
  components: {},
  data() {
    return {
      loading: false,
  ...
  methods: {
    ...
    async search(size, page) {
      ...
      this.loading = true
      try {
        const res = await this.$http.post('/achievement-class-three-personnel/list', newForm)
        if (res.status) {
    ...
    handleView(id) {
      this.$router.push(`/achievement-class-three-personnel-view/${id}`)
    },
    @throttle(1000)
    handleExport() {
