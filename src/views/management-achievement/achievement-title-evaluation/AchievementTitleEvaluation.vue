...
*/
<template>
  <div class="achievement-title-evaluation">
    <el-form
        size="small"
        ref="form"
        inline
...
<script>
export default {
  name: 'AchievementTitleEvaluation',
  components: {},
  data() {
    return {
      loading: false,
  ...
  methods: {
    ...
    async search(size, page) {
      ...
      this.loading = true
      try {
        const res = await this.$http.post('/achievement-title-evaluation/list', newForm)
        if (res.status) {
    ...
    handleView(id) {
      this.$router.push(`/achievement-title-evaluation-view/${id}`)
    },
    @throttle(1000)
    handleExport() {
