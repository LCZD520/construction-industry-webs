...
*/
<template>
  <div class="class-three-personnel-view">
...
<script>
import ClassThreePersonnelOrder from "./class-three-personnel-view/ClassThreePersonnelOrder";
...
export default {
  name: 'ClassThreePersonnelView',
  components: {ImagesUploadView, ClassThreePersonnelOrder, ImagesUpload},
  data() {
    return {
      isShowDetail: false,
  ...
  methods: {
    ...
    async confirmEditRemarks() {
      ...
      try {
        const res = await this.$http.put('/class-three-personnel/update-remarks', {
          id,
          remark: this.remarks
        })
        if (res && res.status) {
      ...
      if (arr.length > 0) {
        try {
          ...
          if (res && res.status) {
            ...
            let rootFolder = zip.folder(`三类人员-${this.form.customerName}`)
            ...
            zip.generateAsync({
              ...
              // 下载的文件名
              let filename = `三类人员-${this.form.customerName}.zip`;
              saveAs(res, filename)
            })
            return
          }
    ...
    getDetailById(_id) {
      this.$http.get('/class-three-personnel/detail/' + _id).then(res => {
        if (res.status && res.data !== null) {
          ...
          this.$store.dispatch('classThreePersonnel/updateClassThreePersonnelDetail', this.form)
        }
      })
    },
    previewImage(row) {
