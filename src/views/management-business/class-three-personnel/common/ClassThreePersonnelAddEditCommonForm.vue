...
*/
<template>
  <div class="class-three-personnel-add-edit-common-form">
...
export default {
  name: 'ClassThreePersonnelAddEditCommonForm',
  ...
  data() {
    ...
    return {
      ...
      mapObj: {
        ...
        "三类类别": "threeCategory",
        "事项性质": "itemNature",
        "闽政通账号": "minZhengTongAccount",
        "闽政通密码": "minZhengTongPassword",
        "代办金额": "agencyAmount"
      },
      fileList: [],
      file: "",
    ...
    handleSubmit() {
      this.$refs.form.validate(async valid => {
        if (valid) {
          ...
          try {
            const res = await this.$http.post('/class-three-personnel/insert', this.form)
            if (res.status) {
...
.class-three-personnel-add-edit-common-form {
  margin: 0 100px;
}
