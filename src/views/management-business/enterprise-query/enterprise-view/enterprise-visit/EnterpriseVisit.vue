  ...
  methods: {
    ...
    getList(_pageSize) {
      ...
      if (undefined !== _pageSize) {
        url = `/enterprise-visit/list?currentPage=${this.pageInfo.currentPage}&pageSize=${_pageSize}&enterpriseId=${this.form.enterpriseId}`
      } else {
        url = `/enterprise-visit/list?currentPage=${this.pageInfo.currentPage}&pageSize=${this.pageInfo.pageSize}&enterpriseId=${this.form.enterpriseId}`
      }
    ...
    getDetail(_id) {
      this.$http.get('/enterprise-visit/detail/' + _id).then(res => {
        if (res.status) {
          this.form = res.data
          return
        }
    ...
    handleConfirm() {
      this.$refs.visitRecordAddEdit.$refs.formData.validate(valid => {
        if (valid) {
          this.visible = false
          if (this.mode === 'add') {
            this.$http.post('/enterprise-visit/insert', this.form).then(res => {
              if (res.status) {
          ...
          this.$http.put('/enterprise-visit/update', this.form).then(res => {
            if (res.status) {
              this.$message.success(res.message)
              this.getList()
              this.visible = false
    ...
    handleDelete(_index, _row) {
      this.$confirm('确定要删除这条记录吗？', '温馨提示', {
        ...
        this.$http.delete('/enterprise-visit/delete/' + _row.id).then(res => {
          if (res.status) {
            this.$message.success(res.message)
            this.getList()
            return
