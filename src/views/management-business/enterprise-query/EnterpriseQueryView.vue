      ...
      if (arr.length > 0) {
        try {
          ...
          if (res && res.status) {
            ...
            let rootFolder = zip.folder(`企业查询-${this.form.enterpriseName}`)
            if (typeSets.has('contract')) {
              rootFolder.folder('合同')
            }
            if (typeSets.has('handoverSheet')) {
              rootFolder.folder('交接单')
            }
            ...
            res.data.forEach(item => {
              ...
              if (item.type === 'handoverSheet') {
                rootFolder.folder('交接单').file(item.name, base64Str, {base64: true})
              }
            ...
            zip.generateAsync({
              ...
              // 下载的文件名
              let filename = `企业查询-${this.form.enterpriseName}.zip`;
              saveAs(res, filename)
            })
            return
          }
