...
export default {
  ...
  data() {
    return {
      ...
      form: {
        qualificationAgencyId: this.$route.query.id / 1,
        bankName: null,
        openBank: '',
        accountName: '',
        bankCardNo: '',
  ...
  methods: {
     ...
     */
    async getList() {
      await this.$http.get('/qualification-agency-transfer/list/' + this.form.qualificationAgencyId).then(res => {
        if (res.status) {
          this.list = res.data
          return
        }
     ...
     */
    getListBankAccounts() {
      this.$http.get('/qualification-agency-bank-account/list/' + this.form.qualificationAgencyId).then(res => {
        if (res.status) {
    ...
    async handleDetail(_index, _row) {
      try {
        const res = await this.$http.get('/qualification-agency-transfer/detail/' + _row.id)
        if (res && res.status) {
    ...
    handleSubmit() {
      this.$refs.accountAddEditDialog.$refs.form.validate(valid => {
        if (valid) {
          if (this.mode === 'add') {
            this.$http.post('/qualification-agency-bank-account/insert', this.form).then(res => {
              if (res.status) {
          ...
          this.$http.put('/qualification-agency-bank-account/update', this.form).then(res => {
            if (res.status) {
              this.innerVisible = false
              this.$message.success(res.message)
              this.getListBankAccounts()
    ...
    handleAccountAddEdit(_type, _id) {
      ...
      if (_type === 'edit') {
        ...
        this.$http.get('/qualification-agency-bank-account/detail/' + _id).then(res => {
          if (res.status) {
            this.form = res.data
            return
          }
    ...
    handleAccountDelete(_id) {
      this.$confirm('确认要删除此账户吗?', '温馨提示', {
        ...
        this.$http.delete('/qualification-agency-bank-account/delete/' + _id).then(res => {
          if (res.status) {
            this.$message.success(res.message)
            this.getListBankAccounts()
            return
    ...
    async applyTransfer() {
      ...
      this.$refs.form.validate(async valid => {
        if (valid) {
          ...
          let newObj = {
            qualificationAgencyId: this.form.qualificationAgencyId,
            accountName,
            bankCardNo,
            bankName,
            openBank,
          ...
          try {
            const res = await this.$http.post('/qualification-agency-transfer/insert', newObj)
            if (res.status) {
              this.getList()
              this.$message.success(res.message)
