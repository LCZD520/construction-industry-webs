...
export default {
  ...
  computed: {
    ...
    waitApplyAmount() {
      return this.$store.state.qualificationAgency.qualificationAgencyDetail.agencyAmount - this.appliedAmount
    },
  },
  methods: {
    /**
    ...
    async getList() {
      this.loading = true
      try {
        const res = await this.$http.get('/qualification-agency-entry/list/' + this.qualificationAgencyDetail.id)
        if (res.status) {
          this.list = res.data
          return
        }
