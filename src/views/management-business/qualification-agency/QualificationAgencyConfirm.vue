...
*/
<template>
  <div class="qualification-agency-confirm">
    <el-form disabled :model="form" label-position="right" label-width="120px">
      ...
      <el-form :model="form2" ref="form" :disabled="false" :rules="rules" label-width="120px">
        <el-form-item prop="confirmResult" label="执行确认结果">
          <el-radio-group v-model="form2.confirmResult">
            <el-radio
        ...
        <el-form-item label="执行确认意见" prop="confirmOpinion">
          <el-input
              maxlength="100"
              show-word-limit
              :rows="3"
              type="textarea"
              v-model.trim="form2.confirmOpinion">
          </el-input>
        </el-form-item>
        <el-form-item label=" " label-width="120px">
          <el-button type="primary" icon="el-icon-plus" v-throttle="handleSave">
            保存
          </el-button>
...
export default {
  ...
  data() {
    return {
      ...
      form2: {
        qualificationAgencyId: this.$route.params.id / 1 || null,
        confirmResult: null,
        confirmOpinion: '',
      },
      rules: {
        confirmResult: [
          {required: true, message: '请选择执行确认结果', trigger: 'blur'}
        ],
        confirmOpinion: [
          {required: false}
        ]
  ...
  methods: {
    async handleSave() {
      this.$refs.form.validate(async valid => {
        ...
        if (valid) {
          try {
            const res = await this.$http.post('/qualification-agency-perform-confirm', this.form2)
            if (res.status) {
