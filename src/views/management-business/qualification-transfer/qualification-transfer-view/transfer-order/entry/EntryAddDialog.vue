...
<template>
  <div class="order-entry">
    <el-dialog width="60%" title="资质转让-入账申请" :visible.sync="visible" :before-close="()=>$emit('closeDialog')">
              ...
              <el-row :gutter="20">
                <el-col :span="12">
                  <el-form-item label="订单编号">
                    <el-input disabled class="width-full" :value="qualificationTransferInfo.orderno">
                    </el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="12">
                  <el-form-item label="收购意向客户">
                    <el-input disabled class="width-full"
                              :value="qualificationTransferInfo.qualificationTransferDO.transferCustomers">
                    </el-input>
                  </el-form-item>
                </el-col>
              </el-row>
                ...
                <el-col :span="12">
                  <el-form-item label="款项用途" prop="fundsPurpose">
                    <el-select class="width-full" v-model="form.fundsPurpose">
                      <el-option
                          v-for="item in $store.state.qualification_transfer_entry_funds_purpose_options"
                          :key="item.value"
                          :label="item.label"
                          :value="item.value">
                      </el-option>
...
export default {
  ...
  data() {
    return {
      form: {
        qualificationTransferOrderId: null,
        fundsPurpose: null,
        transferorInfo: '',
  ...
  computed: {},
  mounted() {
    this.form.qualificationTransferOrderId = this.qualificationTransferOrderId
  },
  methods: {
    inputChange(_index) {
    ...
    handleSubmit() {
      this.$refs.form.validate(async valid => {
        if (valid) {
          ...
          newForm.qualificationTransferCreatorId = this.qualificationTransferInfo.qualificationTransferDO.creatorId
          newForm.qualificationTransferId = this.qualificationTransferInfo.qualificationTransferDO.id
          newForm.transferCustomers = this.qualificationTransferInfo.qualificationTransferDO.transferCustomers
          newForm.qualificationTransferOrderId = this.qualificationTransferInfo.id
          console.log(newForm, "newForm")
          try {
            const res = await this.$http.post('/qualification-transfer-order-entry', newForm)
            if (res.status) {
