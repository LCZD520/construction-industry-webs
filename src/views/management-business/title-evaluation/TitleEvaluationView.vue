...
<script>
import TitleEvaluationOrder from "./title-evaluation-view/TitleEvaluationOrder";
...
export default {
  ...
  components: {ImagesUploadView, TitleEvaluationOrder, ImagesUpload},
  data() {
    return {
      isShowDetail: false,
  ...
  methods: {
    ...
    async confirmEditRemarks() {
      ...
      try {
        const res = await this.$http.put('/title-evaluation/update-remarks', {
          id,
          remark: this.remarks
        })
        if (res && res.status) {
      ...
      if (arr.length > 0) {
        try {
          ...
          if (res && res.status) {
            ...
            let rootFolder = zip.folder(`职称评审-${this.form.customerName}`)
            ...
            zip.generateAsync({
              ...
              // 下载的文件名
              let filename = `职称评审-${this.form.customerName}.zip`;
              saveAs(res, filename)
            })
            return
          }
    ...
    getDetailById(_id) {
      this.$http.get('/title-evaluation/detail/' + _id).then(res => {
        if (res.status && res.data !== null) {
          ...
          this.$store.dispatch('titleEvaluation/updateTitleEvaluationDetail', this.form)
        }
      })
    },
    previewImage(row) {
