...
*/
<template>
  <div class="title-evaluation-place-order-dialog">
    <el-dialog :close-on-click-modal="false" width="80%" title="职称评审下单" :visible.sync="outerVisible"
...
export default {
  name: 'TitleEvaluationPlaceOrderDialog',
  ...
  methods: {
    ...
    getWaitAssessor(_pageSize) {
      ...
      if (undefined !== _pageSize) {
        url = `/title-evaluation/wait-assessor/${this.form.id}?currentPage=${this.pageInfo.currentPage}&pageSize=${_pageSize}`
      } else {
        url = `/title-evaluation/wait-assessor/${this.form.id}?currentPage=${this.pageInfo.currentPage}&pageSize=${this.pageInfo.pageSize}`
      }
    ...
    async confirmOrder() {
      if (this.form.listSelectedIds.length > 0) {
        this.loading = true
        try {
          const res = await this.$http.post('/title-evaluation/place-order', this.form)
          if (res.status) {
