...
*/
<template>
  <div class="title-evaluation-order">
    ...
    <el-dialog
        append-to-body
        width="80%" title="职称评审订单编辑" :visible.sync="orderEditVisible" :before-close="()=>this.orderEditVisible = false">
...
import TitleEvaluationPlaceOrderDialog from "./TitleEvaluationPlaceOrderDialog";
import TitleEvaluationOrderDetail from "./TitleEvaluationOrderDetail";
import EntryAddDialog from "./entry/EntryAddDialog";
import {loading} from "../../../../util/decorator";

export default {
  name: 'TitleEvaluationOrder',
  components: {EntryAddDialog, TitleEvaluationOrderDetail, TitleEvaluationPlaceOrderDialog},
  data() {
    return {
      // 订单详情弹窗是否可见
      detailVisible: false,
  ...
  methods: {
    ...
    async getListOrders(_id) {
      const res = await this.$http.get('/title-evaluation/list-order/' + _id)
    ...
    async entry(_row) {
      ...
      // const res = await this.$http.get('/title-evaluation/list-selected-talent/' + _row.id)
      // if (res.status && res.data !== null) {
    ...
    async handleCancel(id) {
      const res = await this.$http.put(`/title-evaluation-order/order-cancel/${id}`)
      if (res && res.status) {
    ...
    async getListAssessorsByOrderId(id) {
      try {
        const res = await this.$http.get(`/title-evaluation-order/list-assessors/${id}`)
        if (res && res.status) {
    ...
    getWaitAssessor(_pageSize) {
      ...
      if (undefined !== _pageSize) {
        url = `/title-evaluation/wait-assessor/${id}?currentPage=${this.pageInfo.currentPage}&pageSize=${_pageSize}`
      } else {
        url = `/title-evaluation/wait-assessor/${id}?currentPage=${this.pageInfo.currentPage}&pageSize=${this.pageInfo.pageSize}`
      }
    ...
    async getUnselectedWaitAssessor() {
      ...
      try {
        const res = await this.$http.post(`/title-evaluation/unselected-wait-assessor/${this.form.id}`, requestBody)
        if (res.status) {
          this.selectedAssessorList = res.data.list
          this.pageInfo.total = res.data.total
        }
