...
<template>
  <div class="order-entry">
    <el-dialog
        ...
        width="70%" title="学历提升-入账申请" :visible.sync="visible" :before-close="()=>$emit('closeDialog')">
              ...
              <el-row :gutter="20">
                <el-col :span="8">
                  <el-form-item label="客户名称">
                    <el-input disabled :value="$store.state.educationPromotion.educationPromotionDetail.customerName"/>
                  </el-form-item>
                </el-col>
                <el-col :span="8">
                ...
                <el-col :span="8">
                  <el-form-item label="学历提升录入人">
                    <el-select disabled class="width-full"
                               :value="$store.state.educationPromotion.educationPromotionDetail.creatorId">
                      <el-option
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        educationPromotionOrderId: null,
        custormerName: '',
        fundsPurpose: null,
        transferorInfo: '',
  ...
  computed: {
    educationPromotionDetail() {
      return this.$store.state.educationPromotion.educationPromotionDetail
    }
  },
  created() {
  ...
  methods: {
    async getEntryRecordsByOrderId() {
      try {
        const res = await this.$http.get(`/education-promotion-order-entry/detail/${this.orderInfo.id}`)
        console.log(res)
        if (res && res.status) {
          this.list = res.data
        }
    ...
    handleSubmit() {
      ...
      let {customerName, creatorId, id} = this.$store.state.educationPromotion.educationPromotionDetail
      ...
      newForm.educationPromotionId = id
      newForm.educationPromotionOrderId = this.orderInfo.id
      newForm.educationPromotionCreatorId = creatorId
      newForm.orderno = this.orderInfo.orderno
      console.log(newForm, "newForm")
      this.$refs.form.validate(async valid => {
        if (valid) {
          try {
            const res = await this.$http.post('/education-promotion-order-entry/insert', newForm)
            if (res && res.status) {
