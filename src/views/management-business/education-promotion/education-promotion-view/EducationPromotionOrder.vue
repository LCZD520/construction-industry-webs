...
*/
<template>
  <div class="education-promotion-order">
    ...
    <el-dialog
        append-to-body
        width="80%" title="学历提升订单编辑" :visible.sync="orderEditVisible" :before-close="()=>this.orderEditVisible = false">
...
import EducationPromotionPlaceOrderDialog from "./EducationPromotionPlaceOrderDialog";
import EducationPromotionOrderDetail from "./EducationPromotionOrderDetail";
import EntryAddDialog from "./entry/EntryAddDialog";
import {loading} from "../../../../util/decorator";

export default {
  name: 'EducationPromotionOrder',
  components: {EntryAddDialog, EducationPromotionOrderDetail, EducationPromotionPlaceOrderDialog},
  data() {
    return {
      // 订单详情弹窗是否可见
      detailVisible: false,
  ...
  methods: {
    ...
    async getListOrders(_id) {
      const res = await this.$http.get('/education-promotion/list-order/' + _id)
    ...
    async entry(_row) {
      ...
      // const res = await this.$http.get('/education-promotion/list-selected-talent/' + _row.id)
      // if (res.status && res.data !== null) {
    ...
    async handleCancel(id) {
      const res = await this.$http.put(`/education-promotion-order/order-cancel/${id}`)
      if (res && res.status) {
    ...
    async getListAssessorsByOrderId(id) {
      try {
        const res = await this.$http.get(`/education-promotion-order/list-assessors/${id}`)
        if (res && res.status) {
    ...
    getWaitAssessor(_pageSize) {
      ...
      if (undefined !== _pageSize) {
        url = `/education-promotion/wait-assessor/${id}?currentPage=${this.pageInfo.currentPage}&pageSize=${_pageSize}`
      } else {
        url = `/education-promotion/wait-assessor/${id}?currentPage=${this.pageInfo.currentPage}&pageSize=${this.pageInfo.pageSize}`
      }
    ...
    async getUnselectedWaitAssessor() {
      ...
      try {
        const res = await this.$http.post(`/education-promotion/unselected-wait-assessor/${this.form.id}`, requestBody)
        if (res.status) {
          this.selectedAssessorList = res.data.list
          this.pageInfo.total = res.data.total
        }
