...
*/
<template>
  <div class="education-promotion-place-order-dialog">
    <el-dialog :close-on-click-modal="false" width="80%" title="学历提升下单" :visible.sync="outerVisible"
...
export default {
  name: 'EducationPromotionPlaceOrderDialog',
  ...
  methods: {
    ...
    getWaitAssessor(_pageSize) {
      ...
      if (undefined !== _pageSize) {
        url = `/education-promotion/wait-assessor/${this.form.id}?currentPage=${this.pageInfo.currentPage}&pageSize=${_pageSize}`
      } else {
        url = `/education-promotion/wait-assessor/${this.form.id}?currentPage=${this.pageInfo.currentPage}&pageSize=${this.pageInfo.pageSize}`
      }
    ...
    async confirmOrder() {
      if (this.form.listSelectedIds.length > 0) {
        this.loading = true
        try {
          const res = await this.$http.post('/education-promotion/place-order', this.form)
          if (res.status) {
