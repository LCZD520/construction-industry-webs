    ...
    <div class="split-line">
      <div class="split-line-left">
        <el-button icon="el-icon-plus" size="small" type="primary"
                   @click.stop="$router.push('/education-promotion-add')">添加
        </el-button>
      </div>
...
export default {
  ...
  methods: {
    ...
    async search(size, page) {
      ...
      try {
        const res = await this.$http.post('/education-promotion/list', newForm)
        if (res && res.status) {
    ...
    handleEdit(_row) {
      console.log(_row)
      this.$router.push(`/education-promotion-edit/${_row.id}`)
    },
    async recovery(id) {
      try {
        const res = await this.$http.delete(`/education-promotion/recovery/${id}`)
        if (res && res.status) {
    ...
    async handleDelete(id) {
      try {
        const res = await this.$http.delete(`/education-promotion/delete/${id}`)
        if (res && res.status) {
          this.$message.success(res.message)
          await this.search()
          return
