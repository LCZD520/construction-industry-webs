...
*/
<template>
  <div class="order-education-promotion">
    <el-form
        size="small"
        ref="form"
        :rules="rules2"
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template #default="{row}">
          {{ row.educationPromotion && row.educationPromotion.customerName }}
        </template>
      </el-table-column>
      <el-table-column
          min-width="180"
    ...
    <el-dialog
      ...
      <EducationPromotionTransferDetailDialog
...
<script>
import AccountAddEditDialog from "./order-education-promotion/AccountAddEditDialog";
import EducationPromotionTransferDetailDialog from "./order-education-promotion/EducationPromotionTransferDetailDialog";
...
export default {
  name: 'OrderEducationPromotion',
  components: {
    ImagesUploadView,
    ImagesUpload2, EducationPromotionTransferDetailDialog, AccountAddEditDialog
  },
  data() {
  ...
  methods: {
    ...
    async search(size, page) {
      ...
      this.loading = true
      try {
        const res = await this.$http.get('/education-promotion-order/list-all', {
          params: {
    ...
    async getOrderTransfersByOrderId(id) {
      try {
        const res = await this.$http.get(`/education-promotion-transfer/list/${id}`)
        if (res && res.status) {
    ...
    handleView(id) {
      this.$router.push('/order-education-promotion-view/' + id)
    },
    /**
     * 确认订单
     * @param id
     */
    handleConfirm(id) {
      this.$router.push('/order-education-promotion-edit/' + id)
    },
    handleTransfer(row) {
    ...
    async getListBankAccounts(id) {
      try {
        const res = await this.$http.get('/education-promotion-bank-account/list/' + id)
        if (res && res.status) {
    ...
    handleSubmit() {
      this.$refs.accountAddEditDialog.$refs.form.validate(valid => {
        if (valid) {
          if (this.mode === 'add') {
            this.$http.post('/education-promotion-bank-account/insert', this.form).then(res => {
              if (res.status) {
          ...
          this.$http.put('/education-promotion-bank-account/update', this.form).then(res => {
            if (res.status) {
              this.innerVisible = false
              this.$message.success(res.message)
              this.getListBankAccounts(this.form.titleEvaluationOrderId)
    ...
    handleAccountAddEdit(_type, _id) {
      ...
      if (_type === 'edit') {
        ...
        this.$http.get('/education-promotion-bank-account/detail/' + _id).then(res => {
          if (res.status) {
            this.form = res.data
            return
          }
    ...
    handleAccountDelete(id) {
      this.$confirm('确认要删除此账户吗?', '温馨提示', {
        ...
        const res = await this.$http.delete(`/education-promotion-bank-account/delete/${id}`)
        if (res && res.status) {
          this.$message.success(res.message)
      ...
      this.$refs.formTransfer.validate(async valid => {
        if (valid) {
          ...
          listImages.forEach(item => {
            item.namespace = 'education-promotion'
          ...
          try {
            const res = await this.$http.post('/education-promotion-transfer/insert', obj)
            if (res.status) {
              this.$message.success(res.message)
              this.visible = false
              this.$nextTick(() => {
