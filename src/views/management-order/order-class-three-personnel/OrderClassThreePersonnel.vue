...
*/
<template>
  <div class="order-class-three-personnel">
    <el-form
        size="small"
        ref="form"
        :rules="rules2"
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template #default="{row}">
          {{ row.classThreePerson && row.classThreePerson.customerName }}
        </template>
      </el-table-column>
      <el-table-column
          min-width="180"
    ...
    <el-dialog
      ...
      <ClassThreePersonnelTransferDetailDialog
...
<script>
import AccountAddEditDialog from "./order-class-three-personnel/AccountAddEditDialog";
import ClassThreePersonnelTransferDetailDialog
  from "./order-class-three-personnel/ClassThreePersonnelTransferDetailDialog";
...
export default {
  name: 'OrderClassThreePersonnel',
  components: {
    ImagesUploadView,
    ImagesUpload2, ClassThreePersonnelTransferDetailDialog, AccountAddEditDialog
  },
  data() {
  ...
  methods: {
    ...
    async search(size, page) {
      ...
      this.loading = true
      try {
        const res = await this.$http.get('/class-three-person-order/list-all', {
          params: {
    ...
    async getOrderTransfersByOrderId(id) {
      try {
        const res = await this.$http.get(`/class-three-personnel-transfer/list/${id}`)
        if (res && res.status) {
    ...
    handleView(id) {
      this.$router.push('/order-class-three-personnel-view/' + id)
    },
    /**
     * 确认订单
     * @param id
     */
    handleConfirm(id) {
      this.$router.push('/order-class-three-personnel-edit/' + id)
    },
    handleTransfer(row) {
    ...
    async getListBankAccounts(id) {
      try {
        const res = await this.$http.get('/class-three-personnel-bank-account/list/' + id)
        if (res && res.status) {
    ...
    handleSubmit() {
      this.$refs.accountAddEditDialog.$refs.form.validate(valid => {
        if (valid) {
          if (this.mode === 'add') {
            this.$http.post('/class-three-personnel-bank-account/insert', this.form).then(res => {
              if (res.status) {
          ...
          this.$http.put('/class-three-personnel-bank-account/update', this.form).then(res => {
            if (res.status) {
              this.innerVisible = false
              this.$message.success(res.message)
              this.getListBankAccounts(this.form.titleEvaluationOrderId)
    ...
    handleAccountAddEdit(_type, _id) {
      ...
      if (_type === 'edit') {
        ...
        this.$http.get('/class-three-personnel-bank-account/detail/' + _id).then(res => {
          if (res.status) {
            this.form = res.data
            return
          }
    ...
    handleAccountDelete(id) {
      this.$confirm('确认要删除此账户吗?', '温馨提示', {
        ...
        const res = await this.$http.delete(`/class-three-personnel-bank-account/delete/${id}`)
        if (res && res.status) {
          this.$message.success(res.message)
      ...
      this.$refs.formTransfer.validate(async valid => {
        if (valid) {
          ...
          listImages.forEach(item => {
            item.namespace = 'class-three-personnel'
          ...
          try {
            const res = await this.$http.post('/class-three-personnel-transfer/insert', obj)
            if (res.status) {
              this.$message.success(res.message)
              this.visible = false
              this.$nextTick(() => {
