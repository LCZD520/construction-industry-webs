...
*/
<template>
  <div class="entry-qualification-agency">
    <el-form
        ref="form"
        inline
        :rules="rules"
      ...
      <el-form-item label="申请人" label-width="120px" prop="creatorId">
        <el-select size="small" v-model="form.creatorId" placeholder="请选择申请人">
          <el-option
...
export default {
  ...
  methods: {
    ...
    async search(size, page) {
      ...
      try {
        const res = await this.$http.post('/qualification-agency-entry/list', newForm)
        if (res && res.status) {
    ...
    handleView(id) {
      this.$router.push(`/entry-qualification-agency-view/${id}`)
    },
    /**
     * 审核
     * @param _row
