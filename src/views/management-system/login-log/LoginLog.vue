  ...
  <div class="login-log">
    <el-form
      ...
      <el-form-item label="机构名称" prop="mechanismName">
        <el-input size="small" v-model.trim="form.mechanismName" placeholder="请输入机构名称">
        </el-input>
      </el-form-item>
      <el-form-item label=" ">
        <el-button size="small" icon="el-icon-search" @click="search(10,1)" :loading="loading" type="primary">搜 索</el-button>
...
export default {
  ...
  methods: {
    ...
    async search(size, page) {
      ...
      this.loading = true
      try {
        const res = await this.$http.post('/login-log', newForm)
        if (res.status) {
